...
public class ControllerNhaVanHoaQuanLy implements Initializable{
	...
	public void addQuanLy() {
		try {
			...
			ControllerFormAddQuanLyNhaVanHoa.setControllerNhaVanHoaQuanLy(this);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	private void refreshTableViewQuanLy() {
		...
		List<ModelNhaVanHoa> listQuanLy = Connector.getAllQuanLyNhaVanHoaByIdUser(Connector.currentUser.getIdUser());
	...
	public void showDetailQuanLy() {
		...
		try {
			...
			ControllerFormDetailQuanLyNhaVanHoa.setControllerNhaVanHoaQuanLy(this);
